            ...
            myButton_back: {
                ...
                onclick: async function () {
                    if (parentAdCodes.has(adcode.value)) {
                        updateBarByAdCode(parentAdCodes.get(adcode.value));
                    }
                }
            }
        }
    ...
    xAxis: {
        ...
        axisLabel: {
            ...
            formatter:function(value){
                ...
                //console.log(value,adcodeMap2Name.get(value));
                return adcodeMap2Name.get(value)
            }
        }
    },
    yAxis: {
...
function setOption() {
    ...
    option.series = [{
        type: 'bar',
        data: NumberCnts[staticsType.value].map(item => item.value)
    }];
    option.xAxis.data = NumberCnts[staticsType.value].map(item => item.adcode);
}
...
let updateBarByAdCode = async function (newAdCode) {
...
onMounted(() => {
        ...
        return function (params) {
           // console.log(params);
            if (handled) {
                nextTick(async () => {
                    handled = false;
                    if (params.name && !bannedAdCodes.has(params.name)) {
                        parentAdCodes.set(params.name, adcode.value);
                        if (!await updateBarByAdCode(params.name)) {
                            bannedAdCodes.add(params.name);
                        }
                    }
                    setTimeout(() => {
                        handled = true;
    ...
    nextTick(() => {
        updateBarByAdCode(adcode.value);
    });
